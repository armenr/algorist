...
static void init(graph* g, const char* r) {
  init_hashtbl(&found, 128, sdbm_hash);

  init_graph(g, true);  /* directed graph */
  insert_vertex(g, r, strlen(r)+1);
	insert_hashtbl(found, r, strlen(r)+1, r, strlen(r)+1, compare_keys_str);

	init_stack(&solution);
}

static void add_candidate(const char* candidate, int v) {
  insert_vertex(&g, candidate, (int)strlen(candidate)+1);      
  int n = g.nvertices-1;
  insert_edge(&g, v, n, 0);
	insert_hashtbl(found, candidate, strlen(candidate)+1, &n, sizeof(int), 
	  compare_keys_str);
}
  ...
  for(i = 0; i < DICT_SIZE; i++) {
    ...
    if(string_compare(s, t) == 1) { /* doublet*/
      ...
      if(strcmp(candidate, m)) { /* unmatched*/
        if(get_hashtbl(found, candidate) == NULL)  /* not already added*/
