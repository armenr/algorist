...
static char* get_block(void) {
  // block indices always in pairs (start, end)
  int* sb = (int*)de_queue(&bq); // start of block
  int* eb = (int*)de_queue(&bq); // end of block
  int l = (*eb - *sb);  // block length
  char* b = malloc(l);  // extract block using indices //!!!REFACTOR!!!//
  
  strncpy(b, input + *sb, l); // get block
...
static void print_blocks(void) {
  while(!empty_queue(bq)) {
    char* b = get_block(); // function contains malloc, which must be freed
