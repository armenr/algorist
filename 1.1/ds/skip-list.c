...
static skipnode* make_skipnode(int level, int key, const void* item, int sz);
static void insert_node(skipnode* h, skipnode* s, int l);
static void delete_node(skipnode* s);
...
void init_skipset(skipset** ss ) {
  *ss = malloc(sizeof(skipset));
  int i = HEADER;
  (*ss)->header = make_skipnode(MAX_LEVEL, 0, &i, sizeof(i));
  (*ss)->header->key = HEADER; 
  (*ss)->level = 0;
}
...
void print_skipset(const skipset* ss) {
  int i;
...
const skipnode* contains_skipnode(const skipset* ss, int key) {
  skipnode* s = ss->header;
  int i;

  for(i = ss->level; i >= 0; i--) {
    while(s->next[i] != NULL && s->next[i]->key < key) 
      s = s->next[i];
      
    if(s->next[i] != NULL && s->next[i]->key == key)
      return s->next[i];
  }
...
void insert_skipnode(skipset* ss, int key, const void* item, int sz) {
  if(contains_skipnode(ss, key) != NULL) /*no duplicates allowed in set*/
    return;
  ...
  insert_node(ss->header, make_skipnode(l, key, item, sz), l);
}
...
/*node may be freed. */
void delete_skipnode(skipset* ss, int key) {
  skipnode* s = (skipnode*)contains_skipnode(ss, key);  
...
/*To create a SkipNode we must first allocate memory for the node itself then */
/*allocate memory for the array of next pointers. */
...
static void insert_node(skipnode* h, skipnode* s, int l) {
  int i;
  for(i = l; i >= 0; i--) {
    while(h->next[i] != NULL && h->next[i]->key < s->key) 
      h = h->next[i];
