...
static skipnode* make_skipnode(int level, const void* key, size_t key_sz, 
...
void init_skipset(skipset** ss ) {
  *ss = malloc(sizeof(skipset));
  int i = HEADER;
  (*ss)->header = make_skipnode(MAX_LEVEL, &i, sizeof(int), &i, sizeof(int));
  (*ss)->level = 0;
}
...
void insert_skipnode(skipset* ss, const void* key, size_t key_sz, 
  ...
  insert_node(ss->header, make_skipnode(l, key, key_sz, item, item_sz), 
    l, compare_keys);
}
...
static skipnode* make_skipnode(int level, const void* key, size_t key_sz, 
  ...
  sn->item = malloc(item_sz);
  memcpy(sn->item, item, item_sz);
