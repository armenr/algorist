...
void insert_tree(tree** t, const void* key, int key_sz, 
  const void* item, int item_sz, const tree* parent, 
  int (*compare_keys)(const void* key1, const void* key2)) {
  tree* p; 
  ...
  if(*t == NULL) {
    ...
    p->item = malloc(item_sz);  
    p->item_sz = item_sz;
  	memcpy(p->item, item, item_sz);
  ...
  int cmp = compare_keys(key, (*t)->key);
  if(cmp < 0) 
    insert_tree(&((*t)->left), key, key_sz, item, item_sz, *t, compare_keys);
  else
    insert_tree(&((*t)->right), key, key_sz, item, item_sz, *t, compare_keys);
}
...
static void update_node(tree** t, tree* p, 
 ...
 /*overwrite existing node item with new node item */
  free((*t)->item); 
  (*t)->item = malloc(p->item_sz);
  (*t)->item_sz = p->item_sz;
  memcpy((*t)->item, p->item, p->item_sz);
