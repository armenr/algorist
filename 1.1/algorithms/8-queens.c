...
void process_solution(const int a[], void* k) {
  solution_count++;
}

bool is_a_solution(const int a[], int k, void* n) {
  return k == *(int*)n;
}

void make_move(const int a[], int k, void* n) {}
void unmake_move(const int a[], int k, void* n) {}

/*	What are possible elements of the next slot in the 8-queens problem? */
void construct_candidates(const int a[], int k, int n, int c[], 
  int* ncandidates) {
